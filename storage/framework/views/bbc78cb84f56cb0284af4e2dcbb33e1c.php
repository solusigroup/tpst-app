                        ...
                        <td class="text-end">
                            <div class="btn-group btn-group-sm">
                                <?php if($item->status !== 'posted'): ?>
                                    <form method="POST" action="<?php echo e(route('admin.jurnal.post', $item)); ?>" class="d-inline" onsubmit="return confirm('Post jurnal ini?')"><?php echo csrf_field(); ?><button class="btn btn-outline-success" title="Post"><i class="cil-check-circle"></i></button></form>
                                <?php else: ?>
                                    <form method="POST" action="<?php echo e(route('admin.jurnal.unpost', $item)); ?>" class="d-inline" onsubmit="return confirm('Unpost jurnal ini?')"><?php echo csrf_field(); ?><button class="btn btn-outline-warning" title="Unpost"><i class="cil-x-circle"></i></button></form>
                                <?php endif; ?>
                                <a href="<?php echo e(route('admin.jurnal.edit', $item)); ?>" class="btn btn-outline-primary"><i class="cil-pencil"></i></a>
                                <form method="POST" action="<?php echo e(route('admin.jurnal.destroy', $item)); ?>" class="d-inline" onsubmit="return confirm('Yakin hapus?')"><?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?><button class="btn btn-outline-danger"><i class="cil-trash"></i></button></form>
                            </div>
                        </td>
                    </tr>
