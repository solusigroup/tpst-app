            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>Rp <?php echo e(number_format($item->total_wage, 0, ',', '.')); ?></td>
                        <td class="<?php echo e($item->overtime_pay > 0 ? 'text-success' : 'text-muted'); ?>">
                            <?php echo e($item->overtime_pay > 0 ? 'Rp '.number_format($item->overtime_pay, 0, ',', '.') : '-'); ?>

                        </td>
                        <td><strong>Rp <?php echo e(number_format($item->total_wage + $item->overtime_pay, 0, ',', '.')); ?></strong></td>
                        ...
                        <td>
                            ...
                            <?php elseif($item->status == 'approved'): ?> <span class="badge bg-info">Disetujui</span>
                                <div class="small text-muted">Oleh: <?php echo e($item->approvedBy->name ?? '-'); ?></div>
                            <?php elseif($item->status == 'paid'): ?> <span class="badge bg-success">Dibayar</span>
                            <?php endif; ?>
                        </td>
                        <td class="text-end">
                            <div class="btn-group btn-group-sm">
                                <a href="<?php echo e(route('admin.hrd.wage-calculation.show', $item)); ?>" class="btn btn-outline-info" title="Detail"><i class="cil-search"></i></a>
                                
                                <?php if($item->status === 'pending' && auth()->user()->hasRole(['manajemen', 'super_admin'])): ?>
                                <form action="<?php echo e(route('admin.hrd.wage-calculation.approve', $item)); ?>" method="POST" class="d-inline">
                                    <?php echo csrf_field(); ?>
                                ...
                                <button type="button" class="btn btn-outline-primary" title="Bayar" data-coreui-toggle="modal" data-coreui-target="#payModal<?php echo e($item->id); ?>"><i class="cil-money"></i></button>
                                <?php endif; ?>

                                <a href="<?php echo e(route('admin.jurnal.create', ['ref_type' => urlencode('App\Models\WageCalculation'), 'ref_id' => $item->id])); ?>" class="btn btn-outline-secondary" title="Buat Jurnal"><i class="cil-book"></i></a>
                            </div>

                            <?php if($item->status === 'approved'): ?>
                            <!-- Pay Modal -->
                            <div class="modal fade" id="payModal<?php echo e($item->id); ?>" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog modal-sm">
                                    <form action="<?php echo e(route('admin.hrd.wage-calculation.pay', $item)); ?>" method="POST" class="modal-content text-start">
                                        <?php echo csrf_field(); ?>
                                        <div class="modal-header">
                                        ...
                                        <div class="modal-body">
                                            <p>Total: <strong>Rp <?php echo e(number_format($item->total_wage + $item->overtime_pay, 0, ',', '.')); ?></strong></p>
                                            <div class="mb-3">
                                                <label class="form-label">Tanggal Bayar</label>
                                                <input type="date" name="paid_date" class="form-control" required value="<?php echo e(date('Y-m-d')); ?>">
                                            </div>
                                        </div>
                                        <div class="modal-footer">
