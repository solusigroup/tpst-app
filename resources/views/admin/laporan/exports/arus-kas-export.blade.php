...
<table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
    <tbody>
        ...
        <tr>
            ...
            <td style="border: 1px solid #ddd; padding: 5px; text-align: right; color: #198754;">Rp {{ (request('export') == 'excel' ? ($totalKasMasuk) : number_format($totalKasMasuk, 0, ',', '.')) }}</td>
        </tr>
        <tr>
            <td style="border: 1px solid #ddd; padding: 5px; padding-left: 20px;">Pengeluaran Kas</td>
            <td style="border: 1px solid #ddd; padding: 5px; text-align: right; color: #dc3545;">(Rp {{ (request('export') == 'excel' ? ($totalKasKeluar) : number_format($totalKasKeluar, 0, ',', '.')) }})</td>
        </tr>
        <tr>
            <td style="border: 1px solid #ddd; padding: 5px; font-weight: bold;">Kas Bersih dari Aktivitas Operasi</td>
            <td style="border: 1px solid #ddd; padding: 5px; text-align: right; font-weight: bold;">Rp {{ (request('export') == 'excel' ? ($totalKasBersih) : number_format($totalKasBersih, 0, ',', '.')) }}</td>
        </tr>
    </tbody>
</table>
...
<table style="width: 100%; border-collapse: collapse; border-top: 2px solid #333;">
    <tr>
        ...
        <td style="padding: 10px; text-align: right;">Rp {{ (request('export') == 'excel' ? ($saldoAwal) : number_format($saldoAwal, 0, ',', '.')) }}</td>
    </tr>
    <tr>
        <td style="padding: 10px; font-weight: bold;">Kenaikan/(Penurunan) Kas Bersih</td>
        <td style="padding: 10px; text-align: right; {{ $totalKasBersih >= 0 ? 'color: #198754;' : 'color: #dc3545;' }}">Rp {{ (request('export') == 'excel' ? ($totalKasBersih) : number_format($totalKasBersih, 0, ',', '.')) }}</td>
    </tr>
    <tr>
        <td style="padding: 10px; font-weight: bold; font-size: 14px;">SALDO KAS AKHIR PERIODE</td>
        <td style="padding: 10px; text-align: right; font-weight: bold; font-size: 14px;">Rp {{ (request('export') == 'excel' ? ($saldoAkhir) : number_format($saldoAkhir, 0, ',', '.')) }}</td>
    </tr>
</table>
@endsection
