...
@section('title', 'Edit Logbook Mesin')
...
<div class="card col-md-8">
    <div class="card-body">
        <form action="{{ route('admin.machine-logs.update', $machineLog->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('machine_id') is-invalid @enderror" id="machine_id" name="machine_id" required>
                        ...
                        @foreach($machines as $machine)
                            <option value="{{ $machine->id }}" {{ old('machine_id', $machineLog->machine_id) == $machine->id ? 'selected' : '' }}>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('waktu_cek') is-invalid @enderror" id="waktu_cek" name="waktu_cek" required>
                        ...
                        <option value="Engine On" {{ old('waktu_cek', $machineLog->waktu_cek) == 'Engine On' ? 'selected' : '' }}>Engine On (Pagi / Mulai Operasi)</option>
                        <option value="Engine Off" {{ old('waktu_cek', $machineLog->waktu_cek) == 'Engine Off' ? 'selected' : '' }}>Engine Off (Sore / Selesai Operasi)</option>
                    </select>
                    @error('waktu_cek')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        <div class="form-check p-3 border rounded h-100 shadow-sm" style="background-color: rgba(16, 185, 129, 0.05); border-left: 4px solid #10b981 !important;">
                            <input class="form-check-input ms-1" type="radio" name="status_lampu" id="statusHijau" value="Hijau" {{ old('status_lampu', $machineLog->status_lampu) == 'Hijau' ? 'checked' : '' }} required>
                    ...
                    <div class="col-md-6">
                        <div class="form-check p-3 border rounded h-100 shadow-sm" style="background-color: rgba(245, 158, 11, 0.05); border-left: 4px solid #f59e0b !important;">
                            <input class="form-check-input ms-1" type="radio" name="status_lampu" id="statusKuning" value="Kuning" {{ old('status_lampu', $machineLog->status_lampu) == 'Kuning' ? 'checked' : '' }}>
                    ...
                    <div class="col-md-6">
                        <div class="form-check p-3 border rounded h-100 shadow-sm" style="background-color: rgba(6, 182, 212, 0.05); border-left: 4px solid #06b6d4 !important;">
                            <input class="form-check-input ms-1" type="radio" name="status_lampu" id="statusBiru" value="Biru" {{ old('status_lampu', $machineLog->status_lampu) == 'Biru' ? 'checked' : '' }}>
                    ...
                    <div class="col-md-6">
                        <div class="form-check p-3 border rounded h-100 shadow-sm" style="background-color: rgba(239, 68, 68, 0.05); border-left: 4px solid #ef4444 !important;">
                            <input class="form-check-input ms-1" type="radio" name="status_lampu" id="statusMerah" value="Merah" {{ old('status_lampu', $machineLog->status_lampu) == 'Merah' ? 'checked' : '' }}>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('keterangan') is-invalid @enderror" id="keterangan" name="keterangan" rows="3" placeholder="Isi catatan khusus jika status Kuning, Biru, atau Merah">{{ old('keterangan', $machineLog->keterangan) }}</textarea>
                @error('keterangan')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
