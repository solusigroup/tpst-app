...
@section('title', 'Ritase DLH - Disetujui')

@section('content')
<div class="page-header">
    <div>
        <h1 class="d-flex align-items-center gap-2">
            <i class="cil-check-circle text-success"></i>
            Ritase DLH — Disetujui (Approved)
        </h1>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
                <li class="breadcrumb-item active">Ritase DLH (Disetujui)</li>
            </ol>
        </nav>
    </div>
    <div class="d-flex gap-2">
        <a href="{{ route('admin.ritase-dlh.export-excel', array_merge(request()->all(), ['type' => 'approved'])) }}" class="btn btn-success" target="_blank">
            <i class="cil-cloud-download me-1"></i> Ekspor Excel
        </a>
        <a href="{{ route('admin.ritase-dlh.export-pdf', array_merge(request()->all(), ['type' => 'approved'])) }}" class="btn btn-danger" target="_blank">
    ...
    <div class="card-body py-3">
        <form method="GET" class="row g-2 align-items-end">
            ...
            @if(request()->hasAny(['search', 'start_date', 'end_date', 'armada_id']))
                <div class="col-auto">
                    <a href="{{ route('admin.ritase-dlh.approved') }}" class="btn btn-outline-secondary">Reset</a>
                </div>
            @endif
        </form>
    </div>
        ...
        <div class="card border-0 shadow-sm h-100">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <div class="text-body-secondary small mb-1">Total Ritase Disetujui</div>
    ...
    <div class="col-sm-6 col-lg-4">
        <div class="card border-0 shadow-sm h-100">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <div class="text-body-secondary small mb-1">Total Biaya Tipping (Rp 80.000/ton)</div>
                        <div class="fs-4 fw-bold text-warning">Rp {{ number_format($totalBiayaTipping, 0, ',', '.') }}</div>
                    </div>
                    <div class="p-2 bg-warning bg-opacity-10 rounded-3">
                        <i class="cil-dollar fs-4 text-warning"></i>
                    </div>
                </div>
            </div>
        </div>
...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <strong><i class="cil-check-circle text-success me-2"></i>Daftar Ritase DLH — Disetujui (Belum Dibayar)</strong>
        <span class="badge bg-success fs-6">{{ number_format($totalCount, 0, ',', '.') }} Ritase</span>
    </div>
                ...
                <tbody>
                    ...
                    <tr>
                        <td colspan="13" class="text-center py-5 text-body-secondary">
                            ...
                            Tidak ada data ritase DLH yang disetujui pada filter ini.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
